...
def get_crop_img(img: np.ndarray, imgarea) -> np.ndarray:
    if imgarea == IMGAREA.MSGLOG:
        ...
        pass
    return maintain_aspect_ratio_resize(
        img[y : y + h, x : x + w], width=wr, height=hr, inter=a
    )
    ...
    for i in range(3, 101, 2):
        for j in range(1, 100):
            ...
            img = cv2.adaptiveThreshold(
                img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, i, j
            )
            # inverse -> black letters on white background
            img = cv2.bitwise_not(img)
            # threshold
